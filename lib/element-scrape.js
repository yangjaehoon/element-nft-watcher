...
export async function findRarityMatches(browser, listings, opts) {
  ...
  // 후보를 한 번에 병렬로 열면(Promise.all) 헤드리스 브라우저 페이지가 동시에
  // 여러 개(많으면 수십 개) 떠서 컬렉션 개수가 늘어날수록 리소스를 다 먹어
  // 다른 컬렉션까지 "Navigation timeout"으로 실패하는 걸 실제로 겪었다. 순서대로
  // 하나씩 확인한다 — 느리지만 안정적이다. 캐시(cache)가 있으면 대부분은
  // 상세 페이지를 다시 열지 않고 바로 끝난다.
  const checked = [];
  for (const l of candidates) {
    ...
    try {
      ...
      checked.push({ listing: l, traitValue });
    } catch (e) {
      console.error(`[findRarityMatches] #${l.tokenId} 등급 확인 실패`, e.message);
    }
  }
