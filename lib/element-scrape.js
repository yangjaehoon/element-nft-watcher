...
function parseSearchResult(json) {
  ...
  // 필터 URL로 직접 이동한 경우 totalCount 가 비어있을 수 있어, 그럴 땐 받은 건수로 대체한다.
  const totalCount = json?.data?.search?.totalCount ?? listings.length;
...
export async function fetchAssetTrait(browser, contractAddress, tokenId, traitName = "Rarity") {
  ...
  try {
    await page.goto(`https://element.market/assets/bsc/${contractAddress}/${tokenId}`, {
      waitUntil: "networkidle2",
      timeout: 20000,
    });
